<template>
  <div class='mytree'>
    <el-tree class="custom-tree-node" :data="treeData.tree" ref="tree" :node-key="treeData.nodeKey" :flag="treeData.hideParentCheckBox" :accordion="treeData.accordion" :expand-on-click-node="treeData.expandClickNode" :highlight-current="treeData.highlightCurrent" :indent="treeData.treeIndent" :show-checkbox="treeData.treeShowCheckbox" :check-strictly="treeData.treeCheckStrictly" @node-click="nodeClick" @check="check" @check-change="checkChange" :previewJson="previewJson">
      <div class="custom-tree-node span-ellipsis" slot-scope="{ node, data }" :id="'nodeId' + node.id" @mouseenter="enter('nodeId' + node.id, data, node)" @mouseleave="leave('nodeId' + node.id, data)">
        	<!-- <span v-if="!data.children||data.id=='0'">
						<img :src="defaultClose" alt />
					</span> -->
					<!-- 有子元素显示的图标 -->
					<!-- <span v-else>
        ...
        <span class="hoverIcon span_label_show" v-if="treeData.editable" style="display:none">
          <span class="el-icon-circle-plus-outline opButton" style="color: #409EFF" @click.stop="() => appendNode(node, data)" title="添加标签"></span>
          <span class="el-icon-edit-outline opButton"  style="color: #409EFF" @click.stop="editNode(node, data)" title="修改标签"></span>
          <span class="el-icon-delete opButton" style="color: #409EFF" @click.stop="() => removeNode(node, data)" title="删除"></span>
        </span>
      </div>
    </el-tree>
...
export default {
  ...
  data() {
    return {
      defaultOpen: require("../assets/image/boxPlus.png"),
			defaultClose: require("../assets/image/boxLose.png"),
      treeData: {
        tree: [
          {
            id: 1,
