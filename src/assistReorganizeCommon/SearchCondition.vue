<template>
	<div :conditions="conditionItems">
		<el-form ref="myFormItem" class="formItem" :label-width="labelWidth || '90px'" label-position="left" label-suffix=":">
			<slot v-for="(item, index) in conditions">
				<el-form-item class="formLine" :label="item.name" :key="item.key" :dict="item.dict" :prop="item.prop" :myRequired="item.required || '0'" style="margin-bottom: 0px; position: relative; float:left; width: 100%;">
					<div :style="item.style || {
          height: '40px',
          overflow: 'hidden',
          float: 'left'
          }">
						<el-checkbox v-if="item.type === 'checkbox' && item.checkAll !== undefined" v-model="item.checkAll" label="全部" :indeterminate="item.isIndeterminate" name="type" :size="item.size || 'mini'" @change="handleCheckAllChange(item)"></el-checkbox>
						<slot v-for="d in item.items">
							<slot v-if="!item.hideItems || (item.hideItems.indexOf(d) === -1)">
								<slot v-if="d.type === 'checkbox' || item.type === 'checkbox' ">
									<el-checkbox @change="handleCheckedChange(item)" v-model="item.value" :label="d.name || d" name="type" :size="item.size || 'mini'"></el-checkbox>
								</slot>
								<slot v-else-if="d.type === 'radio' || item.type === 'radio' ">
									<el-radio @change="$emit('onChange', conditions, 'radio', item)" :value="d.value|| d" v-model="item.value" :label="d.value|| d" :size="item.size || 'mini'" name="type">{{d.name || d}}</el-radio>
								</slot>
								<slot v-else-if="d.type === 'switch' || item.type === 'switch' ">
									<el-form-item :label="d.name" prop="delivery">
										<span v-if="d">{{d}}</span>
										<el-switch @change="$emit('onChange', conditions, 'switch', item)" active-color="#13ce66" inactive-color="#ff4949" :size="item.size || 'mini'"></el-switch>
									</el-form-item>
								</slot>
								<slot v-else-if="d.type === 'dateRange' || item.type === 'dateRange' ">
									<el-date-picker type="daterange" unlink-panels start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['', '']" :size="item.size || 'mini'" v-model="item.value" :picker-options="item.pickerOptions" value-format="yyyy-MM-dd" @change="$emit('onChange', conditions, 'dateRange', item)"></el-date-picker>
								</slot>
								<slot v-else-if="d.type === 'tags' || item.type === 'tags' ">
									<el-tag class="eTagItem" v-for="tag in item.value" closable :key="item.value" :type="tag.type" :size="item.size || 'mini'" @close="handleRemoveTag(item, tag)" style="margin-left: 5px;">{{tag.name}}</el-tag>
								</slot>
								<slot v-else-if="d.type === 'text' || item.type === 'text'">
									<el-input :size="item.size || 'mini'" v-model="item.value" :disabled="d.disabled || item.disabled" :placeholder="d.placeholder || item.placeholder" @change="$emit('onChange', conditions, 'input', item.value)"></el-input>
								</slot>
								<slot v-else-if="d.type === 'select' || item.type === 'select' ">
									<el-select :size="item.size || 'mini'" v-model="item.value" :multiple="item.multiple" :disabled="d.disabled || item.disabled" :placeholder="d.placeholder || item.placeholder" @change="$emit('onChange', conditions, 'select', item.value)">
										<el-option v-for="tItem in d.options" :label="tItem.label || tItem" :value="tItem.value || tItem" :key="tItem.value"></el-option>
									</el-select>
								</slot>
								<slot v-else-if="d.type === 'textarea' || item.type === 'textarea'">
									<el-input type="textarea" v-model="item.value" :disabled="d.disabled || item.disabled" :size="item.size || 'mini'" :placeholder="d.placeholder" @change="$emit('onChange', conditions, 'textarea', item.value)"></el-input>
								</slot>
								<slot v-else-if="d.type === 'datePicker' || item.type === 'datePicker'">
									<el-date-picker v-model="item.value" :size="item.size || 'mini'" :format="d.format || item.format || 'yyyy-MM-dd HH:mm:ss'" :value-format="d['value-format'] || item['value-format'] || 'yyyy-MM-dd HH:mm:ss'" :type="(d.pickerType || item.pickerType) || 'datetime'" :placeholder="(d.placeholder || item.placeholder) || '选择日期时间'" @change="$emit('onChange', conditions, 'datePicker', item.value)"></el-date-picker>
								</slot>
							</slot>
						</slot>
					</div>
